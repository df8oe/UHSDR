...
{
	bool retval =  (config->divider == 4 || config->divider == 6 || (config->divider >= 8 && config->divider <= 900 ));
	// TODO: Check rDiv to be power of 2 and in range 1..128

	if (retval == true)
	{
		retval = ((config->mult >= 15 && config->mult <= 90) && (config->num <= MAX_UINT20) && (config->denom <= MAX_UINT20));
	}

	return retval;
}

static bool Si5351a_CalculateConfig(uint32_t frequency, Si5351a_Config_t* config)
{
	...
	if (divider_max <= 900)
	{
		config->divider = divider_max;
	}
	else if (divider_min >= 4)
	{
		config->divider = divider_min;
	}
	else
	{
		config->divider = 0;
	...
	if (config->divider % 2)
	{
		config->divider--;						// Ensure an even integer division ratio
	}

	pllFreq = config->divider * frequency;					// Calculate the pllFrequency: the divider * desired output frequency

	config->mult = pllFreq / xtalFreq;						// Determine the multiplier to get to the required pllFrequency
	l = pllFreq % xtalFreq;							// It has three parts:
	f = l;											// mult is an integer that must be in the range 15...90
	f *= MAX_UINT20;									// num and denom are the fractional parts, the numerator and denominator
	f /= xtalFreq;									// each is 20 bits (range 0...1048575)
	config->num = f;										// the actual multiplier is  mult + num / denom
	config->denom = MAX_UINT20;								// For simplicity we set the denominator to the maximum 1048575


	return Si5351a_ValidateConfig(config);
}
...
static bool Si5351a_ApplyConfig(Si5351a_Config_t* config)
{
	...
	bool result = Si5351a_SetupPLL(SI5351_SYNTH_PLL_A, config->mult, config->num, config->denom);
	...
	if (result == true)
	{
		result = Si5351a_SetupMultisynthInteger(SI5351_SYNTH_MS_0, config->divider, SI5351_R_DIV_1);
#ifdef TEST_QUADRATURE
		result |= Si5351a_SetupMultisynthInteger(SI5351_SYNTH_MS_1, config->divider, SI5351_R_DIV_1);
#endif
	}
...
#ifdef TEST_QUADRATURE
		Si5351a_WriteRegister(SI5351_CLK1_PHASE_OFFSET, config->divider);
#endif

		Si5351a_WriteRegister( SI5351_CLK0_CONTROL, SI5351_OUTPUT_ON);
#ifdef TEST_QUADRATURE
		Si5351a_WriteRegister( SI5351_CLK1_CONTROL, SI5351_OUTPUT_ON);
#endif
...
static Oscillator_ResultCodes_t Si5351a_PrepareNextFrequency(uint32_t freq, int temp_factor)
{
	...
	freq /= 4;
#endif
	return Si5351a_CalculateConfig(freq, &si5351a_state.next) == true?OSC_OK:OSC_TUNE_IMPOSSIBLE;
}
...
void Si5351a_Init()
{
	...
	si5351a_state.current.divider = 0;
	si5351a_state.next.divider = 0;
